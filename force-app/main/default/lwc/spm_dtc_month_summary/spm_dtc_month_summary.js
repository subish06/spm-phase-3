...
import getTargetBDM from '@salesforce/apex/DemoMonth.getTargetBDM';
import getBDMTargetData from '@salesforce/apex/DemoMonth.getBDMTargetData';
import getVarianceBDMData from '@salesforce/apex/DemoMonth.getVarianceBDMData';
import getActualTarget from '@salesforce/apex/DemoMonth.getActualsBDM';
import getBDMActualData from '@salesforce/apex/DemoMonth.getBDMActualData';
import getVarianceBDM from '@salesforce/apex/DemoMonth.getVarianceBDM';
...
export default class Spm_dtc_month_summary extends LightningElement {
    ...
    connectedCallback() {
        // this.addEventListener('monthsummaryevent', this.handleChildEvntMonthSummary.bind(this));
        // this.addEventListener('monthsummaryevent2', this.handleChildEvntMonthSummary.bind(this));
    }
    ...
    @wire(getTargetBDM, { financialYear: '$financialYear', currentUser: '$currentBDD', selectedSubChannel: '$selectedSubChannel' })
    ...
    @wire(getActualTarget, { financialYear: '$financialYear', currentUser: '$currentBDD', selectedSubChannel: '$selectedSubChannel' })
    ...
    @wire(getVarianceBDM, { financialYear: '$financialYear', currentUser: '$currentBDD', selectedSubChannel: '$selectedSubChannel' })
    ...
    handleInputChange(event) {
        this.updatedValue = event.target.value ? event.target.value : 0;
        // let updatedValue = event.target.value ? event.target.value : 0;

        if (this.updatedValue) {
            this.updatedValue = parseFloat(this.updatedValue);
        }
        ...
        this.salesTargetData = this.salesTargetData.map(record => {
            if (record.key === recordKey) {
                console.log('record.value===>',  { ...record, [fieldName]: this.updatedValue, isUpdated: true });
                return { ...record, [fieldName]: this.updatedValue, isUpdated: true };
            }
            console.log('record out-=--->', record)
            return record;
        });
    }
    ...
    handleInputChange(event) {
        ...
        console.log('this.updatedValue====>',this.updatedValue);
    ...
    handleSave() {
        ...
        saveBDMTargets({ bddId: this.currentBDD, subChannel: this.selectedSubChannel, financialYear: this.financialYear, updatedRecords})
        .then(result => {
            showSuccess('Target Updated Successfully');
            this.closePopup();
            this.loading = false;
            this.getBDMTargetData();
            this.getBDMVarianceData();
            this.getActualData();
